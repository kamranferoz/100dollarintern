jQuery(function($) {
    ...
    $('#quickad-services-wrapper').on('click', '#quickad-delete', function(e) {
        if (confirm('Are you sure?')) {
            ...
            data['ids[]'] = services;
            $.post(ajaxurl+'?action=delPostType', data, function(response) {
                if(response != 0) {
            ...
            $services.sortable({
                ...
                update : function( event, ui ) {
                    ...
                    $.ajax({
                        type : 'POST',
                        url  : ajaxurl,
                        data : { action: 'quickad_update_post_type_position', position: data },
    ...
    $(document).on('click', '.quickad-type-edit', function(e) {
        ...
        $.post(ajaxurl+'?action=langTranslation_PostTypes', data, function(response) {
            if(response != "") {
    ...
    $('#modal_LangTranslation').on('click', '#saveEditLanguage', function() {
        ...
        $.ajax({
            type: "POST",
            data: data,
            url: ajaxurl+'?action=edit_langTranslation_PostTypes',
