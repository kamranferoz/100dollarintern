jQuery(function($) {
    ...
    $('#quickad-services-wrapper').on('click', '#quickad-delete', function(e) {
        if (confirm('Are you sure?')) {
            ...
            data['ids[]'] = services;
            $.post(ajaxurl+'?action=delSalaryType', data, function(response) {
                if(response != 0) {
            ...
            $services.sortable({
                ...
                update : function( event, ui ) {
                    ...
                    $.ajax({
                        type : 'POST',
                        url  : ajaxurl,
                        data : { action: 'quickad_update_salary_type_position', position: data },
    ...
    $(document).on('click', '.quickad-type-edit', function(e) {
        e.stopPropagation();
        e.preventDefault();
        ...
        $.post(ajaxurl+'?action=langTranslation_SalaryTypes', data, function(response) {
            if(response != "") {
    ...
    $('#modal_LangTranslation').on('click', '#saveEditLanguage', function() {
        ...
        $.ajax({
            type: "POST",
            data: data,
            url: ajaxurl+'?action=edit_langTranslation_SalaryTypes',
