...
//Check For An Update
$getVersions = file_get_contents('https://bylancer.com/api/quickjob-release-versions.php') or die ('ERROR');
if ($getVersions != '')
{
    ...
    foreach ($versionList as $aV)
    {
        if ( $aV > $config['version']) {
            ...
            if ( !is_file(  $update_dir.'QUICKJOB-CMS-'.$aV.'.zip' )) {

                if ( !is_dir( $update_dir ) ) mkdir ( $update_dir );
                ?>
                <form id="upload" method="post" action="upload.php" enctype="multipart/form-data">
                    <div id="drop">
                        Drop Here <br>QUICKJOB-CMS-<?php echo $aV ?>.zip
                        <a>Browse</a>
            ...
            if (isset($_GET['doUpdate']) && $_GET['doUpdate'] == true) {
                ...
                $zipHandle = zip_open($update_dir.'QUICKJOB-CMS-'.$aV.'.zip');
                ...
                while ($aF = zip_read($zipHandle) )
                {
                    ...
                    if ( !is_dir($installable_dir.$thisFileName) ) {
                        ...
                        else if(strpos($filename, 'lang_') !== false) // update language files
                        {
                            ...
                            $tempLang = fopen ('temp_lang.php','w');
                            fwrite($tempLang, $contents);
                            fclose($tempLang);

                            $new_lang = getLanguageFileVariable(ROOTPATH.'/admin/temp_lang.php');
                            ...
                            unlink('temp_lang.php');
                            echo' <span class="label label-warning">UPDATED</span></li>';
                        }
                        else
                        {
                ...
                // Close the file
                fclose($handle);
                unlink($update_dir.'QUICKJOB-CMS-'.$aV.'.zip');
                //unlink($update_dir);
    ...
    if (isset($updated) && $updated == true) {
        ...
        echo '<script>window.location = "'.$config['site_url'].'admin";</script>';
    }
