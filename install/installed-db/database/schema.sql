...
--
-- Host: 127.0.0.1
-- Generation Time: Nov 30, 2019 at 09:53 AM
...
CREATE TABLE IF NOT EXISTS `<<prefix>>logs` (
  ...
  `log_details` longtext COLLATE utf8_unicode_ci,
  PRIMARY KEY (`log_id`)
...
CREATE TABLE IF NOT EXISTS `<<prefix>>product` (
  ...
  `custom_values` longtext CHARACTER SET utf8,
  `created_at` datetime DEFAULT NULL,
  `updated_at` datetime DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `<<prefix>>product_type` (
  ...
  `translation_name` longtext CHARACTER SET utf8 COLLATE utf8_unicode_ci,
...
CREATE TABLE IF NOT EXISTS `<<prefix>>subadmin1` (
  ...
  KEY `name` (`name`),
  KEY `active` (`active`)
) ENGINE=MyISAM AUTO_INCREMENT=3732 DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;
...
CREATE TABLE IF NOT EXISTS `<<prefix>>subadmin2` (
  ...
  KEY `name` (`name`),
  KEY `active` (`active`)
) ENGINE=MyISAM AUTO_INCREMENT=42584 DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;
...
CREATE TABLE IF NOT EXISTS `<<prefix>>user` (
  ...
  `user_type` enum('user','employer') NOT NULL,
...
CREATE TABLE `<<prefix>>user_applied` (
  `id` int(10) NOT NULL AUTO_INCREMENT,
  `user_id` int(10) DEFAULT NULL,
