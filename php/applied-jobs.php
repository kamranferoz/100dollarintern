...
LEFT JOIN `".$config['db']['pre']."companies` c on p.company_id = c.id
 WHERE p.status = 'active' AND p.hide = '0' AND p.id = '".$applied['job_id']."' ";
    ...
    // Output to template
    $page = new HtmlTemplate ('templates/' . $config['tpl_name'] . '/applied-jobs.tpl');
    $page->SetParameter ('OVERALL_HEADER', create_header($lang['APPLIED_JOBS']));
    ...
    $page->SetParameter ('APPLIEDJOBS', $total_item);
