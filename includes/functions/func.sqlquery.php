    ...
    if($config['lang_code'] != 'en' && $config['userlangsel'] == '1'){
        if($info['translation_lang'] != '' && $info['translation_name'] != ''){
            ...
            if($trans_name != ''){
                ...
                $custom_fields_title = stripslashes($info['title']);
            }
        }
    }else{
        $custom_fields_title = stripslashes($info['title']);
    ...
    foreach ($result as $info)
    {
        ...
        if($info['custom_type'] == 'checkboxes')
        {
            ...
            foreach($options as $key4=>$value4)
            {
                ...
                $Checkbox .= '<div class="checkbox"><input type="checkbox" name="custom['.$info['custom_id'].'][]" id="'.$value4.$j.'" value="'.$value4.'" '.$selected.' />';
...
function get_items($userid=null,$status=null,$premium=false,$page=null,$limit=null,$sort="p.id",$location=false,$order=false,$sort_order="DESC"){
    ...
    if($premium){
        if($where == '')
            $where .= "where g.show_on_home = 'yes' and (p.featured = '1' or p.urgent = '1' or p.highlight = '1')";
        else
            $where .= " AND g.show_on_home = 'yes' and (p.featured = '1' or p.urgent = '1' or p.highlight = '1')";
    }
   ...
   if($order){
    ...
    $query = "SELECT p.*, u.group_id, g.show_on_home, c.id company_id, c.name company_name, c.logo company_image
FROM `".$config['db']['pre']."product`  p
LEFT JOIN `".$config['db']['pre']."user` u ON u.id = p.user_id
LEFT JOIN `".$config['db']['pre']."usergroups` g ON g.group_id = u.group_id
