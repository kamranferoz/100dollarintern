...
function email_template($template,$user_id=null,$password=null,$product_id=null,$item_title=null){
    ...
    if($template == "ad_approve"){
        ...
        $ad_link = $config['site_url']."job/".$product_id;
...
function stripUnwantedTagsAndAttrs($html_str){
    ...
    $allowed_tags = array("div", "h1", "h2", "h3", "h4", "h5", "b", "br", "em", "hr", "i", "p", "s", "a", "img", "span", "table", "tr", "td", "strong", "code", "pre", "ul", "li", "ol", "blockquote", "iframe");
    //List the attributes you want to allow here
    $allowed_attrs = array ("href", "src", "frameborder", "width", "height");
        ...
        if(!empty($config['purchase_key'])){
            ...
            if (!$output['success']) {
                if(isset($config['quickad_secret_file']) && $config['quickad_secret_file'] != ""){
                    if(file_exists(ROOTPATH.'/admin/'.$config['quickad_secret_file'].'.php')){
                        unlink(ROOTPATH.'/admin/'.$config['quickad_secret_file'].'.php');
                    }
